...
class Book:
    """Hanji '<http://hanchi.ihp.sinica.edu.tw/ihp/hanji.htm>'_ Dataset.
    ...
    Methods:
        fetch_data(URL): fetch book bs4 obj from a start page URL of a Book in Hanji
        ...
        char_word_counts(char, limits=(1,4)): count the number of occurrences of a phrase with a certain character
        extract_rare_chars(driver_path, normalization=True): extract rare char in every passages. Note that this function will run for a long time.
    ...
    def _pretty_html(self, soup):
        """cut off irrelevant content, such as side columns in the webpage, from the Hanji HTML source page. 
    ...
    def fetch_data(self, URL, pages_limit=10000, print_bookmark=False, BASE_URL='http://hanchi.ihp.sinica.edu.tw/ihpc/', sleep_range=(1, 3)):
        '''fetch book bs4 obj from a start page URL of a Book in Hanji
        
        Args:
            URL (string): the start page url from Hanji website
    ...
    def strip_all_irrelevant_tags(self, connect_the_broken_lines=True, html_cutoff=True):
        ...
        self.strip_tag("span", attrs={'id':re.compile(r"^q\d+")})  # remove commentaries & collation notes together
        self.strip_tag("span",  attrs={"style":"display:none;width:;height:;color:red;font-size:13px"}) # "red" strips collation notes
        self.strip_tag("span",  attrs={"style":"display:;width:;height:;color:green;font-size:13px"}) # "green" strips commentary
        self.strip_tag("span",  attrs={"style":"display:none;width:;height:;color:green;font-size:13px"}) # "green" strips commentary from Hanshu, and some collation notes elsehwere (TODO: tidy up)
    ...
    def write_htmls(self, path='data/', index=0, html_cutoff=False):
        ...
        Args:
            ...
            html_cutoff (bool) : whether or not you want to cut off irrelevant contents in Hanji webpage 
        '''
        try:
            os.makedirs(path)
        except OSError:
